...
public class StatusConditionTemplate extends Template {
    ...
    public boolean immune(Pokemon target) {
        ...
        if (target.getAbility().shouldActivate(AbilityActivation.TryStatusConditionOnUser) &&
            (boolean) target.getAbility().activate(target, null, null, null, null, 0, new StatusCondition(this, null, 0, null, null), null, null, 0, true, AbilityActivation.TryStatusConditionOnUser)) {
            return true;
        }
                ...
                if (!target.isDummy()) {
                    ...
                    if (causer != null && causer != target) {
                        if (causer.getAbility().shouldActivate(AbilityActivation.StatusConditionOnTarget)) {
                            causer.getAbility().activate(causer, target, causingMove, null, null, 0, createdCondition, null, null, 0, true, AbilityActivation.StatusConditionOnTarget);
                        }

                        if (target.getAbility().shouldActivate(causingMove, AbilityActivation.StatusConditionOnUser)) {
                            target.getAbility().activate(target, causer, causingMove, null, null, 0, createdCondition, null, null, 0, true, AbilityActivation.StatusConditionOnUser);
                        }
                    }
                }
