...
public class Type {
    ...
    public Type(Type original, Object source, boolean suppressed) { // copy suppressed
        this.template = original.template;
        this.name = original.name;
        this.superEffective = original.superEffective;
        this.notVeryEffective = original.notVeryEffective;
    ...
    public TypeTemplate[] getSuperEffective(Move move, boolean trueEffectiveness) {
        ...
        if (!trueEffectiveness) {
            ...
            if (move != null &&
                ...
                typeCopy.setSuperEffective((TypeTemplate[]) move.getUser().getAbility().activate(move.getUser(), pokemon, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.CallOpponentSuperEffective));
            }

            if (pokemon.getAbility().shouldActivate(move, AbilityActivation.CallUserSuperEffective)) {
                typeCopy.setSuperEffective((TypeTemplate[]) pokemon.getAbility().activate(pokemon, null, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.CallUserSuperEffective));
            }
            ...
            if (move != null) {
                if (move.primaryShouldActivate(MoveEffectActivation.CallSuperEffective)) {
                    typeCopy.setSuperEffective((TypeTemplate[]) move.activatePrimary(move.getUser(), pokemon, typeCopy, null, 0, null, true, MoveEffectActivation.CallSuperEffective));
                }

                if (move.getUser().getAbility().shouldActivate(AbilityActivation.ChangeOpponentSuperEffective)) {
                    typeCopy.setSuperEffective((TypeTemplate[]) move.getUser().getAbility().activate(move.getUser(), pokemon, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.ChangeOpponentSuperEffective));
                }
    ...
    public TypeTemplate[] getNotVeryEffective(Move move, boolean trueEffectiveness) {
        ...
        if (!trueEffectiveness) {
            ...
            if (move != null &&
                ...
                typeCopy.setNotVeryEffective((TypeTemplate[]) move.getUser().getAbility().activate(move.getUser(), pokemon, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.CallOpponentNotVeryEffective));
            }

            if (pokemon.getAbility().shouldActivate(move, AbilityActivation.CallUserNotVeryEffective)) {
                typeCopy.setNotVeryEffective((TypeTemplate[]) pokemon.getAbility().activate(pokemon, null, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.CallUserNotVeryEffective));
            }
            ...
            if (move != null) {
                if (move.primaryShouldActivate(MoveEffectActivation.CallNotVeryEffective)) {
                    typeCopy.setNotVeryEffective((TypeTemplate[]) move.activatePrimary(move.getUser(), pokemon, typeCopy, null, 0, null, true, MoveEffectActivation.CallNotVeryEffective));
                }

                if (move.getUser().getAbility().shouldActivate(AbilityActivation.ChangeOpponentNotVeryEffective)) {
                    typeCopy.setNotVeryEffective((TypeTemplate[]) move.getUser().getAbility().activate(move.getUser(), pokemon, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.ChangeOpponentNotVeryEffective));
                }
            }
        }
    ...
    public TypeTemplate[] getIneffective(Move move, boolean trueEffectiveness) {
        ...
        if (!trueEffectiveness) {
            ...
            if (move != null &&
                ...
                typeCopy.setIneffective((TypeTemplate[]) move.getUser().getAbility().activate(move.getUser(), pokemon, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.CallOpponentIneffective));
            }

            if (pokemon.getAbility().shouldActivate(move, AbilityActivation.CallUserIneffective)) {
                typeCopy.setIneffective((TypeTemplate[]) pokemon.getAbility().activate(pokemon, null, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.CallUserIneffective));
            }
            ...
            if (move != null) {
                if (move.primaryShouldActivate(MoveEffectActivation.CallIneffective)) {
                    typeCopy.setIneffective((TypeTemplate[]) move.activatePrimary(move.getUser(), pokemon, typeCopy, null, 0, null, true, MoveEffectActivation.CallIneffective));
                }

                if (move.getUser().getAbility().shouldActivate(AbilityActivation.ChangeOpponentIneffective)) {
                    typeCopy.setIneffective((TypeTemplate[]) move.getUser().getAbility().activate(move.getUser(), pokemon, move, typeCopy, null, 0, null, null, null, 0, true, AbilityActivation.ChangeOpponentIneffective));
                }
            }
        }
