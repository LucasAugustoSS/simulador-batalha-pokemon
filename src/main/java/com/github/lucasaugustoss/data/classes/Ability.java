...
public class Ability {
    ...
    public Object activate(Pokemon self, Pokemon opponent, Move move, Type type, Damage damage, int hit, StatusCondition statusCondition, FieldCondition fieldCondition, Stat stat, int statChangeStages, boolean showMessages, AbilityActivation condition) {
        if (App.battleStarted && (
            exclusiveUser == null || pokemon.compare(exclusiveUser, true)
        )) {
            for (AbilityEffect effect : effects) {
                if (effect.shouldActivate(move, condition)) {
                    Object result = effect.activate(this, self, opponent, move, type, damage, hit, statusCondition, fieldCondition, stat, statChangeStages, showMessages, condition);

                    if (condition != AbilityActivation.AfterActivation &&
                        shouldActivate(AbilityActivation.AfterActivation)) {
                        activate(self, opponent, move, type, damage, hit, statusCondition, fieldCondition, stat, statChangeStages, showMessages, AbilityActivation.AfterActivation);
                    }
    ...
    public boolean shouldActivate(Move move, AbilityActivation condition) {
        ...
        if (move.targetsOpponent() && move.getUser() != pokemon && ignorable) {
            ...
            if (move.getUser().getAbility().shouldActivate(AbilityActivation.IgnoreAbility) &&
                (boolean) move.getUser().getAbility().activate(move.getUser(), pokemon, move, null, null, 0, null, null, null, 0, true, AbilityActivation.IgnoreAbility)) {
                return false;
            }
        }
